...
// Desktop prefers the bundled `algoplan` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/casiusss/algoplan/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "algoplan.exe" : "algoplan";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `algoplan-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `algoplan` binary. If one is already present at
