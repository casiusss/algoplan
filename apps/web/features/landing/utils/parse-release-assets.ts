 ...
 *   algoplan-desktop-{version}-mac-{arch}.{dmg|zip}
 *   algoplan-desktop-{version}-windows-{arch}.exe
 *   algoplan-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^algoplan-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
