    ...
    if (isDesktopHandoff) {
      ...
      api
        ...
        .then(({ token }) => {
          ...
          window.location.href = `algoplan://auth/callback?token=${encodeURIComponent(token)}`;
        })
        .catch((err) => {
          setDesktopError(
  ...
  if (isDesktopHandoff && user) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <Card className="w-full max-w-sm">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl">Opening AlgoPlan</CardTitle>
            <CardDescription>
              {desktopToken
                ? "You should see a prompt to open the AlgoPlan desktop app. If nothing happens, click the button below."
          ...
          <CardContent className="flex justify-center">
            {desktopToken ? (
              <Button
                variant="outline"
                onClick={() => {
                  window.location.href = `algoplan://auth/callback?token=${encodeURIComponent(desktopToken)}`;
                }}
              >
                Open AlgoPlan Desktop
              </Button>
            ) : (
