...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("ALGOPLAN_SERVER_URL", DefaultServerURL)
	...
	claudePath := envOrDefault("ALGOPLAN_CLAUDE_PATH", "claude")
	if _, err := exec.LookPath(claudePath); err == nil {
		agents["claude"] = AgentEntry{
			Path:  claudePath,
			Model: strings.TrimSpace(config.GetEnv("ALGOPLAN_CLAUDE_MODEL")),
		}
	}
	codexPath := envOrDefault("ALGOPLAN_CODEX_PATH", "codex")
	if _, err := exec.LookPath(codexPath); err == nil {
		agents["codex"] = AgentEntry{
			Path:  codexPath,
			Model: strings.TrimSpace(config.GetEnv("ALGOPLAN_CODEX_MODEL")),
		}
	}
	opencodePath := envOrDefault("ALGOPLAN_OPENCODE_PATH", "opencode")
	if _, err := exec.LookPath(opencodePath); err == nil {
		agents["opencode"] = AgentEntry{
			Path:  opencodePath,
			Model: strings.TrimSpace(config.GetEnv("ALGOPLAN_OPENCODE_MODEL")),
		}
	}
	openclawPath := envOrDefault("ALGOPLAN_OPENCLAW_PATH", "openclaw")
	if _, err := exec.LookPath(openclawPath); err == nil {
		agents["openclaw"] = AgentEntry{
			Path:  openclawPath,
			Model: strings.TrimSpace(config.GetEnv("ALGOPLAN_OPENCLAW_MODEL")),
		}
	}
	hermesPath := envOrDefault("ALGOPLAN_HERMES_PATH", "hermes")
	if _, err := exec.LookPath(hermesPath); err == nil {
		agents["hermes"] = AgentEntry{
			Path:  hermesPath,
			Model: strings.TrimSpace(config.GetEnv("ALGOPLAN_HERMES_MODEL")),
		}
	}
	geminiPath := envOrDefault("ALGOPLAN_GEMINI_PATH", "gemini")
	if _, err := exec.LookPath(geminiPath); err == nil {
		agents["gemini"] = AgentEntry{
			Path:  geminiPath,
			Model: strings.TrimSpace(config.GetEnv("ALGOPLAN_GEMINI_MODEL")),
		}
	}
	piPath := envOrDefault("ALGOPLAN_PI_PATH", "pi")
	if _, err := exec.LookPath(piPath); err == nil {
		agents["pi"] = AgentEntry{
			Path:  piPath,
			Model: strings.TrimSpace(config.GetEnv("ALGOPLAN_PI_MODEL")),
		}
	}
	cursorPath := envOrDefault("ALGOPLAN_CURSOR_PATH", "cursor-agent")
	if _, err := exec.LookPath(cursorPath); err == nil {
		agents["cursor"] = AgentEntry{
			Path:  cursorPath,
			Model: strings.TrimSpace(config.GetEnv("ALGOPLAN_CURSOR_MODEL")),
		}
	}
	copilotPath := envOrDefault("ALGOPLAN_COPILOT_PATH", "copilot")
	if _, err := exec.LookPath(copilotPath); err == nil {
		agents["copilot"] = AgentEntry{
			Path:  copilotPath,
			Model: strings.TrimSpace(config.GetEnv("ALGOPLAN_COPILOT_MODEL")),
		}
	}
	kimiPath := envOrDefault("ALGOPLAN_KIMI_PATH", "kimi")
	if _, err := exec.LookPath(kimiPath); err == nil {
		agents["kimi"] = AgentEntry{
			Path:  kimiPath,
			Model: strings.TrimSpace(config.GetEnv("ALGOPLAN_KIMI_MODEL")),
		}
	}
	if len(agents) == 0 {
	...
	pollInterval, err := durationFromEnv("ALGOPLAN_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("ALGOPLAN_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("ALGOPLAN_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	maxConcurrentTasks, err := intFromEnv("ALGOPLAN_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via ALGOPLAN_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(config.GetEnv("ALGOPLAN_DAEMON_ID"))
	...
	// — e.g. ~/.algoplan/daemon.id (default) plus ~/.algoplan/profiles/<x>/
	...
	if workspacesRoot == "" {
		...
		if err != nil {
			return Config{}, fmt.Errorf("resolve home directory: %w (set ALGOPLAN_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
	...
	keepEnv := config.GetEnv("ALGOPLAN_KEEP_ENV_AFTER_TASK") == "true" || config.GetEnv("ALGOPLAN_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := config.GetEnv("ALGOPLAN_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("ALGOPLAN_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("ALGOPLAN_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("ALGOPLAN_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid ALGOPLAN_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("ALGOPLAN_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
