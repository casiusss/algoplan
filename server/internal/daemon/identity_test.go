...
func TestEnsureDaemonID_Persists(t *testing.T) {
	...
	path := filepath.Join(home, ".algoplan", "daemon.id")
...
func TestEnsureDaemonID_SharedAcrossProfiles(t *testing.T) {
	...
	// only source of truth is ~/.algoplan/daemon.id.
	profileFile := filepath.Join(home, ".algoplan", "profiles", "staging", "daemon.id")
...
func TestEnsureDaemonID_PromotesPreChangeProfileFile(t *testing.T) {
	...
	profileDir := filepath.Join(home, ".algoplan", "profiles", "staging")
	...
	data, err := os.ReadFile(filepath.Join(home, ".algoplan", "daemon.id"))
	if err != nil {
...
func TestEnsureDaemonID_RegeneratesCorruptFile(t *testing.T) {
	...
	dir := filepath.Join(home, ".algoplan")
...
func TestLegacyDaemonUUIDs_ScansProfileDirs(t *testing.T) {
	...
	for name, id := range map[string]string{"prod": uuidA, "desktop-multica": uuidB} {
		dir := filepath.Join(home, ".algoplan", "profiles", name)
		if err := os.MkdirAll(dir, 0o755); err != nil {
	...
	corruptDir := filepath.Join(home, ".algoplan", "profiles", "corrupt")
