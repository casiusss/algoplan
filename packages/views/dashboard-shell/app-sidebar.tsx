...
// "Inbox" is intentionally NOT in personalNav — the NotificationsBadge owns
// the Inbox row (it composes its own SidebarMenuItem + SidebarMenuButton plus
// the destructive mini-badge). See substitution #2 in 04-02-PLAN.md.
...
/**
 * Brand-colored vertical AccentBar rendered as the FIRST flex child of every
 * SidebarMenuButton. Phase 2 atom reuse (RESEARCH Pattern 5 — leading flex
 * child option). Visibility flips on `isActive` via opacity so the layout
 * never shifts when the user clicks between rows.
 */
function ActiveAccentBar({ isActive }: { isActive: boolean }) {
  return (
    <AccentBar
      color="brand"
      orientation="vertical"
      className={cn(
        "absolute inset-y-1 left-0 h-auto w-0.5",
  ...
  return (
    ...
    >
      <SidebarMenuButton
        ...
        className={cn(
          "relative text-muted-foreground hover:not-data-active:bg-sidebar-accent/70 data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground",
          isDragging && "pointer-events-none",
        )}
      >
        <ActiveAccentBar isActive={isActive} />
        {iconNode}
        <span
...
interface AppSidebarProps {
  /**
   * Workspace id, passed by the parent (apps wire `useCurrentWorkspace()` at
   * the layout level where the WorkspaceSlugProvider is guaranteed). When
   * undefined, the sidebar root falls back to its own `useCurrentWorkspace()`
   * call — see UI-SPEC §2 wsId pass-through rule and SC#4. Sub-components
   * (PriorityGrid, NotificationsBadge) NEVER call the useWorkspaceId hook.
   */
  wsId?: string;
  /** Rendered above SidebarHeader (e.g. desktop traffic light spacer). Distinct from DashboardShell's outer topSlot for DragStrip — this one mounts INSIDE the sidebar (Pitfall 2 — dual topSlot pattern). */
...
export function AppSidebar({
  ...
  // Sidebar root may call useCurrentWorkspace() per UI-SPEC §2 — sub-components
  // receive the resolved wsId as a prop instead.
  const currentWorkspace = useCurrentWorkspace();
  const resolvedWsId = wsId ?? currentWorkspace?.id;
  const p = useWorkspacePaths();
  const { data: workspaces = EMPTY_WORKSPACES } = useQuery(workspaceListOptions());
  const { data: myInvitations = EMPTY_INVITATIONS } = useQuery(myInvitationListOptions());

  const hasRuntimeUpdates = useMyRuntimesNeedUpdate(resolvedWsId);
  const { data: pinnedItems = EMPTY_PINS } = useQuery({
    ...pinListOptions(resolvedWsId ?? "", userId ?? ""),
    enabled: !!resolvedWsId && !!userId,
  });
    ...
    <Sidebar variant="inset">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger
                render={
                  <SidebarMenuButton>
                    <WorkspaceAvatar
                      name={currentWorkspace?.name ?? WordmarkText.charAt(0)}
                      size="sm"
                    />
                    <span className="flex-1 truncate font-medium">
                      <Wordmark workspaceName={currentWorkspace?.name} />
                    </span>
                  </SidebarMenuButton>
                }
              />
              <DropdownMenuContent
                ...
                sideOffset={4}
              >
                {/* UI-SPEC §3: small AlgoPlan identity header at top of the dropdown */}
      ...
      <SidebarContent>
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu className="gap-0.5">
              {/* Inbox row owns its own SidebarMenuItem via NotificationsBadge */}
              <NotificationsBadge wsId={resolvedWsId} />
                ...
                return (
                  <SidebarMenuItem key={item.key}>
                    <SidebarMenuButton
                      ...
                      className="relative text-muted-foreground hover:not-data-active:bg-sidebar-accent/70 data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground"
                    >
                      <ActiveAccentBar isActive={isActive} />
                      <item.icon />
        ...
        <SidebarGroup>
          ...
          <PriorityGrid wsId={resolvedWsId} />
        </SidebarGroup>
