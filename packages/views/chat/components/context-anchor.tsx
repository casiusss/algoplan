...
import type { ContextAnchor } from "@algoplan/core/chat";
...
import { inboxListOptions } from "@algoplan/core/inbox/queries";
...
export function ContextAnchorButton() {
  ...
  const tooltipText = isDisabled
    ? "Nothing to share with AlgoPlan on this page"
    : focusMode && candidate
      ? candidate.type === "issue"
        ? `AlgoPlan knows you're viewing ${candidate.label} · Click to turn off`
        : `AlgoPlan knows you're viewing project "${candidate.label}" · Click to turn off`
      : "Let AlgoPlan know what you're viewing";
      ...
      <TooltipTrigger
        render={
          <Button
            ...
            disabled={isDisabled}
            aria-label={
              focusMode ? "Stop sharing current page" : "Share current page with AlgoPlan"
            }
...
export function ContextAnchorCard() {
  ...
  const tooltipText =
    candidate.type === "issue"
      ? `AlgoPlan knows you're viewing ${candidate.label}${candidate.subtitle ? ` — ${candidate.subtitle}` : ""}`
      : `AlgoPlan knows you're viewing project "${candidate.label}"`;
