...
// Mock @algoplan/views/navigation (AppLink + useNavigation)
vi.mock("../../navigation", () => ({
...
vi.mock("@algoplan/core/issues/stores/issues-scope-store", () => ({
...
vi.mock("@algoplan/core/issues/stores/recent-issues-store", () => ({
...
describe("IssuesPage (shared)", () => {
  ...
  it("shows empty state when there are no issues", async () => {
    ...
    await screen.findByText("Noch keine Issues");
    expect(screen.getByText("Erstelle ein Issue, um zu starten.")).toBeInTheDocument();
  });
