...
import { Popover, PopoverTrigger, PopoverContent } from "@algoplan/ui/components/ui/popover";
import { Checkbox } from "@algoplan/ui/components/ui/checkbox";
import { Command, CommandDialog, CommandInput, CommandList, CommandEmpty, CommandGroup, CommandItem } from "@algoplan/ui/components/ui/command";
import { AvatarGroup, AvatarGroupCount } from "@algoplan/ui/components/ui/avatar";
import { ActorAvatar } from "../../common/actor-avatar";
import type { UpdateIssueRequest, IssueStatus, IssuePriority, TimelineEntry, Issue } from "@algoplan/core/types";
import { ALL_STATUSES, STATUS_CONFIG, PRIORITY_ORDER, PRIORITY_CONFIG } from "@algoplan/core/issues/config";
import { StatusIcon, PriorityIcon, StatusPicker, DueDatePicker, AssigneePicker, canAssignAgent } from ".";
...
import { issueListOptions, issueDetailOptions, childIssuesOptions, issueUsageOptions } from "@algoplan/core/issues/queries";
import { memberListOptions, agentListOptions } from "@algoplan/core/workspace/queries";
import { useUpdateIssue, useDeleteIssue } from "@algoplan/core/issues/mutations";
import { useRecentIssuesStore } from "@algoplan/core/issues/stores";
import { useIssueTimeline } from "../hooks/use-issue-timeline";
import { useIssueReactions } from "../hooks/use-issue-reactions";
import { useIssueSubscribers } from "../hooks/use-issue-subscribers";
import { ReactionBar } from "@algoplan/ui/components/common/reaction-bar";
...
import { timeAgo } from "@algoplan/core/utils";
...
interface IssueDetailProps {
  ...
  onDelete?: () => void;
  /**
   * Phase 6 DTL-04: when defined, the IssueDetailFooter renders the Fertig
   * button which calls this handler. Modal callers (e.g. a future popover)
   * pass a close handler. Routed-page callers (Next.js / Electron router)
   * omit it so only Esc / browser back closes.
   */
  onClose?: () => void;
...
export function IssueDetail({ issueId, onDelete, onClose, defaultSidebarOpen = true, layoutId = "algoplan_issue_detail_layout", highlightCommentId }: IssueDetailProps) {
  const id = issueId;
    ...
    <div className="space-y-5">
        ...
        {propertiesOpen && <div className="space-y-0.5 pl-2">
          ...
          <PropRow label="Priorität">
            <IssuePrioritySegmentedControl
              ...
              onChange={(priority) => handleUpdateField({ priority })}
            />
          </PropRow>
          <PropRow label="Verantwortlich">
            <AssigneePicker assigneeType={issue.assignee_type} assigneeId={issue.assignee_id} onUpdate={handleUpdateField} align="start" />
          </PropRow>
          <PropRow label="Fällig">
      ...
      <div>
        ...
        {detailsOpen && <div className="space-y-0.5 pl-2">
          ...
          <PropRow label="Erstellt">
            <span className="text-muted-foreground">{shortDate(issue.created_at)}</span>
          </PropRow>
          <PropRow label="Aktualisiert">
      ...
      <div className="flex h-full flex-col">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-auto">
                ...
                {/*
                 * Mobile-only Delete affordance (Phase 6 DTL-04 mobile fallback):
                 * the desktop modal-footer hides on mobile, so Delete stays here
                 * for narrow viewports. Per UI-SPEC §Sub-Phase DTL §Mobile fallback.
                 */}
                {isMobile && (
                  <>
                    <DropdownMenuSeparator />
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>Issue löschen?</AlertDialogTitle>
                  <AlertDialogDescription>
                    Dieses Issue und alle Kommentare werden dauerhaft gelöscht. Diese Aktion kann nicht rückgängig gemacht werden.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
        ...
        <div className="mx-auto w-full max-w-4xl px-8 py-8">
          ...
          {parentIssue && (
            ...
            >
              <span className="font-medium shrink-0">Unter-Issue von</span>
              <StatusIcon status={parentIssue.status} className="h-3.5 w-3.5 shrink-0" />
          ...
          {childIssues.length === 0 && (
            <div className="mt-6">
              ...
              >
                <Plus className="h-3.5 w-3.5" />
                <span>+ Unter-Issue hinzufügen</span>
              </button>
            </div>
          )}
          {childIssues.length > 0 && (() => {
        ...
        {/*
         * Phase 6 DTL-04 modal-footer band.
         * Sibling of the scrollable container — sticks to the bottom of the
         * left pane's flex column. Hidden on mobile per UI-SPEC §Sub-Phase
         * DTL §Mobile fallback (Delete stays in More-actions on mobile).
         */}
        {!isMobile && (
          <IssueDetailFooter
