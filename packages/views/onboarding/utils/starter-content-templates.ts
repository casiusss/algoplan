...
export function buildWelcomeIssueText(
  ...
  const sharedInstructions = [
    ...
    `2. **Explain how we work together in AlgoPlan**:`,
  ...
  switch (q.use_case) {
    case "coding":
      return {
        title: "👋 Welcome to AlgoPlan — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AlgoPlan. They plan to use you mostly for **coding work**.\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 Welcome to AlgoPlan — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AlgoPlan. They want your help with **planning and breaking down work**.\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 Welcome to AlgoPlan — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AlgoPlan. They'll use you for **research and writing** — drafting, summarizing, analysis.\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 Welcome to AlgoPlan — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AlgoPlan. They're **exploring** what AlgoPlan can do — no specific goal yet.\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 Welcome to AlgoPlan — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AlgoPlan. ${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 Welcome to AlgoPlan — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using AlgoPlan.\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**Every issue has a right-side panel** called **Properties**. From there you control who works on what. Agents in AlgoPlan are triggered when an issue has:`,
        ``,
  ...
  if (q.team_size === "team") {
    tier2.push({
      ...
      description: [
        `AlgoPlan works best when a small team shares agents.`,
        ``,
        `**Where to find it**:`,
...
export function buildSelfServeSubIssues(
  ...
  // Without a runtime + an agent, nothing else in AlgoPlan works. These two
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        ...
        `1. Go to [github.com/casiusss/algoplan/releases/latest](https://github.com/casiusss/algoplan/releases/latest) and download the \`.dmg\` for macOS`,
        ...
        `   \`\`\``,
        `   curl -fsSL https://raw.githubusercontent.com/casiusss/algoplan/main/scripts/install.sh | bash`,
        `   \`\`\``,
...
export function buildImportPayload({
  ...
  return {
    ...
    project: {
      ...
      description:
        "A few things to try in AlgoPlan. Work through them at your own pace.",
      icon: "👋",
    },
